...
var _host = 'http://img.vjifen.com:8000/images/vma/';
var iconFlag = true;
...
function initSign() {
    ...
    imgUrl = '/v/hnqp-zunxiang/';
    iconFlag = true;
    weekSignFlag = sessionStorage.weekSignFlag == 'undefined' ? '0' : sessionStorage.weekSignFlag;
...
function sign() { //混合签到功能  //本次活动，红包总计{{total}}个，先到先得
    var tpl_1 = '<div class="swiper">' +
        '<p class="signTotal" style="display:{{display}};">签到成功，将会有机会获得红包呦~</p>' +
    ...
    var tpl_2 = '<div class="swiper">' +
        ...
        '</div>' +
        '</div>';
    var tpl_2_0 = '<div class="swiper">' +
        '<p class="signTotal" style="display:{{display}};">签到成功，将会有机会获得红包呦~</p>' +
        '<div class="more" style="display: block;">' +
        '<p class="p-line-3">很遗憾</p>' +
        '<p class="p-line-3">您与大奖擦肩而过</p>' +
        '<p class="p-line-3">下次早点呦</p>' +
        '</div>' +
        '</div>';
    ...
    if (weekSignFlag === '1') { //开启签到功能，则显示开关按钮
        ...
        for (var i = 0; i < signCogAry.length; i++) {
            ...
            if (signCogAry[i].signEarnFlag == '0') { //未达成
                ...
                if (signCogAry[i].prizeUpperLimit == '0' || signCogAry[i].prizeUpperLimit == 'undefined' || signCogAry[i].prizeUpperLimit == '' || signCogAry[i].prizeUpperLimit == undefined) {
                    if (signCogAry[i].signEarnMoney == '0.00') {
                        tpl = tpl_2_0.replace('{{display}}', 'none');
                    } else {
                    ...
                    if (signCogAry[i].signEarnMoney == '0.00') {
                        tpl = tpl_2_0.replace('{{display}}', 'block');
                    } else {
                        tpl = tpl_2.replace('{{money}}', signCogAry[i].signEarnMoney)
        ...
        if (signCogAry[0].periodType == '0') { //周
            $('.bgTop').attr('src', imgUrl + 'img/sign/bg_top.png?v=0.01');
        } else if (signCogAry[0].periodType == '1') { //月
            $('.bgTop').attr('src', imgUrl + 'img/sign/bg_top_2.png?v=0.01');
        }
        //默认的活动说明内容
