(function() {
	$(document).ready(function() {
		var CODE_URL = vge.hbqp + '/DBTHBQPInterface/user/getCaptcha';
		var INIT_URL = vge.hbqp + '/DBTHBQPInterface/sweep/getFailCount';
		var SEND_URL = vge.hbqp + '/DBTHBQPInterface/sweep/serialCode';
				...
				if(jo.result.code === '0') {
					...
					switch(jo.result.businessCode) {
						...
						case '4': // 4 - 活动未开始
							...
							$('.title').html('活动未开始');
							$('.content').html('亲再等等哦~');
							$('.batch').html(jo.reply.batchName + '<br />服务热线：15321413736');
							break;
						case '5': // 5 - 活动已结束
							$('.slogan').css('display', 'none');
							$('.mask').css('display', 'block');
