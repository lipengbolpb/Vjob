    ...
    function cbk(jo) {
        if (jo.result.code == '0') {
            if(jo.result.businessCode=='30'){//大奖核销
                ...
                location.replace('http://' + location.host + '/v/'+ project +'/verification.html?bizcode=' + jo.result.businessCode);
                return false;
            }
            if(jo.reply){
                // 产品类型：0瓶、1罐
            ...
            switch (jo.result.businessCode) {
                ...
                case '11': // 自己重复扫，普通奖
                    ...
                    location.replace('http://' + location.host + '/v/'+ project +'/getcash?bizcode=' + jo.result.businessCode);
                    break;
                case '12': // 
