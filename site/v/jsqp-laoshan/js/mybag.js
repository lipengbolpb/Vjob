(function() {
	...
	function attentioned() {
		...
		function onepage_note(currentpage, cb) {
			var javai = vge.jsqp + '/DBTJSQPInterface/gifts/queryAllGiftsList';
			// alert(javai);
			var req = {
				"openid": openid,
				"hbopenid": hbopenid,
				...
				if(jo.result.code === '0') {
					if(jo.result.businessCode === '0') {
						...
						if(Number(jo.reply.totalMoney) < 1) {
							$('#btn').unbind();
							$('#btn').css({
								'background': 'url(/v/jsqp-laoshan/img/button_dis.png?v=1) no-repeat center',
	...
	if(sessionStorage.mybag=='mybag'){
		$('#attention').css('display','block');
	}
	$('#attention').on('click',function(){
		location.href = 'http://'+location.host+'/v/jsqp-laoshan/attention.html';
	})
