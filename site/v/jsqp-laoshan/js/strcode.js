(function() {
	$(document).ready(function() {
		var CODE_URL = vge.jsqp + '/DBTJSQPInterface/user/getCaptcha';
		var INIT_URL = vge.jsqp + '/DBTJSQPInterface/sweep/getFailCount';
		var SEND_URL = vge.jsqp + '/DBTJSQPInterface/sweep/serialCode';
		var APPID = vge.jsqpappid;
		var PROJECT = 'jsqp-common';
				...
				if(jo.result.code === '0') {
					...
					switch(jo.result.businessCode) {
						...
						case '4': // 4 - 活动未开始
							...
							$('.title').html('活动未开始');
							$('.content').html('亲再等等哦~');
							$('.batch').html(jo.reply.batchName + '<br />服务热线：18519813990');
							break;
						case '5': // 5 - 活动已结束
							$('.slogan').css('display', 'none');
							$('.mask').css('display', 'block');
