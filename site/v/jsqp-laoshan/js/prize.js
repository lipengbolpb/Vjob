(function(){
	...
	if(sessionStorage.grandPrizeType=='1'||sessionStorage.grandPrizeType=='1'){
		$('.prize').attr('src','/v/jsqp-laoshan/img/prize_1.png');
		$('.pic_tip').attr('src','/v/jsqp-laoshan/img/border_1.png?v=2');
		$('.address_box').css('display','block');
	}else{
		$('.prize').attr('src','/v/jsqp-laoshan/img/prize_2.png');
		$('.pic_tip').attr('src','/v/jsqp-laoshan/img/border_2.png?v=2');
	}
	...
	$('#rule').on('click',function(){
		location.replace("https://mp.weixin.qq.com/s/MEe299-y-yN7HLtHIj6h7Q");
	});
	...
	if (sessionStorage.username!=''&&sessionStorage.idcard!=''&&sessionStorage.phonenum!='') {
		...
		$('#btn').css({'background': 'url(/v/jsqp-laoshan/img/button_success.png?v=1) no-repeat center','-webkit-background-size': 'auto 100%'});
	...
	function getCheckCode(cb) { // 获取手机验证码
		var javai = vge.jsqp + '/DBTJSQPInterface/user/getCaptcha';
		var req = {
			"phonenum":$('#tel').val(),
			"sendtype":"0"
		};
   	...
   	function sub_message () { // 提交信息
   		var javai = vge.jsqp + '/DBTJSQPInterface/user/savePrize';
		var req = {
