(function() {
	...
	var params = {
		APPID: vge.csqpappid, //公众号appid
		PROJECT: 'csqp20180130', //项目名称
		SPACK_PORT: vge.csqp + '/DBTHuNanQPInterface/gifts/getGiftspack', //提现接口
		RULE_URL: 'https://mp.weixin.qq.com/s?__biz=MzA4MzMxMDgyNA==&mid=300054164&idx=1&sn=258a6ec5dd97a135ad3e2a02e5477a4e&chksm=0beeb7133c993e05354a0c7b72f21295ada71e8e19ff2f52892513f87eaf9d248acefc9b1a44#rd', //活动规则
		EXPLAIN_URL: '' //隐私说明
	}
				...
				if(weekSignPopup == 1 && weekSignEarnFlag != 1) { //每天首次 且未领取签到红包就会弹出提示
					if(weekSignEarnFlag == 2) {
						...
						$('.getSignCash').on('click', function() {
							...
							$('.content').css('display', 'block');
							$('.sign_title').attr('src', '/v/csqp20180130/img2/sign_title_2.png');
							$('.sign_logo').css({
								'background': 'url(/v/csqp20180130/img2/sign_ed.png) no-repeat right',
								'-webkit-background-size': ' auto 100%'
							});
							$('.sign_tip,#weekSignEarnMoney').css('display', 'block');
							$('.sign_cash').css({
								'background': 'url(/v/csqp20180130/img2/sign_cash_open.png) no-repeat bottom',
								'-webkit-background-size': '100% auto'
							});
						});
					} else {
	...
	//隐藏黄晓明！显示拆红包页面
	$('.home-blastLeft').on('webkitAnimationEnd', function(){
		$('.home').fadeOut();
		$('.get').fadeIn();
	});
