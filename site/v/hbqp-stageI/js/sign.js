...
var APPID = vge.zzhnqpappid;
var PROJECT = 'zzhnqp';
var imgUrl = '/v/zzhnqp/';
var _host = vge.zzhnqpimg + '/DBTHNPZQPplatform';
var iconFlag = true;
...
function sign() { //混合签到功能
    var tpl_1 = '<div class="swiper">' +
        ...
        '<img src="/v/fjqp-common2.0/img/sign/hb.png" class="signHb" style="display: block;">' +
        '</div>';
    ...
    if (weekSignFlag === '1') { //开启签到功能，则显示开关按钮
        ...
        for (var i = 0; i < signCogAry.length; i++) {
            ...
            if (signCogAry[i].signEarnFlag == '0') {
                //未达标
                $('#sign_logo').css('background-image', 'url("' + imgUrl + 'img/sign/sign_logo.png?v=2")');
                iconFlag = false;
        ...
        $('.signClose').on('click', function (event) {
            if (iconFlag) {
                $('#sign_logo').css('background-image', 'url("' + imgUrl + 'img/sign/sign_logo_2.png?v=2")');
            } else {
                $('#sign_logo').css('background-image', 'url("' + imgUrl + 'img/sign/sign_logo.png?v=2")');
            }
            $('.sign').css('display', 'none');
