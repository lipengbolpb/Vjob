(function() {
	...
	var RULE_URL =
		'https://mp.weixin.qq.com/s?__biz=MzI5NjQ4MDMyNg==&mid=100001182&idx=1&sn=6e8c2a427e01fd9153ec15f9cfa8836a&chksm=6c42feb75b3577a11e62b7d89833121f2568b3feb7a0fc261215a1cf35bcd1031d27146145d9#rd';
	// var EXPLAIN_URL = '';

	sessionStorage.isAdAlert = true;
	/* 定义各项参数 */
	var currentMoney = sessionStorage.currentMoney=='undefined'?'0.00':sessionStorage.currentMoney,
		totalAccountMoney = sessionStorage.totalAccountMoney=='undefined'?'0.00':sessionStorage.totalAccountMoney,
		currentVpoints = sessionStorage.currentVpoints == 'undefined'?'0':sessionStorage.currentVpoints,//扫码所得积分
	...
	if (bizcode == 11 || again == 'true') {
		...
		if (sessionStorage.earnTime == '' || sessionStorage.earnTime == undefined) {
			$('.time').html('您已扫过这箱酒<br>并获得了￥' + currentMoney + '元，' + currentVpoints + '积分')
		} else {
			$('.time').html('您已于' + earnTime + '扫过这箱酒<br>并获得了￥' + currentMoney + '元，' + currentVpoints + '积分')
		}
		...
		if (Number(currentMoney) == 0 && Number(currentVpoints) == 0) {
			// $('.title').text('再喝一箱试试看');
			// $('.main').text('您离红包只差一点点');

			$('.main').text('您不在活动区域内');
			
			$(".main").css({"fontSize":"1.4rem","top":"2.6rem"});
		} else {
			$('.title').text('恭喜您获得');
			$('.main').html('<div class="gethb"><div class="money">'+ currentMoney + '<span class="unit">元</span></div><div class="jifen">'+ currentVpoints + '<span class="unit">积分</span></div></div>');
			// $('.main').html('￥' + currentMoney + '<span class="yuan">元</span>');
		}
		if(new Date().getTime()>new Date(2019,5,28).getTime()&&new Date().getTime()<new Date(2019,6,3).getTime()&&sessionStorage.dayScanNum<2){//6.28-7.2 每天一次
			setTimeout(function(){
