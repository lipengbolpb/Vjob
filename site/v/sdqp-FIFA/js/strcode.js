(function () {
    $(document).ready(function () {
        var CODE_URL = vge.sdqp + '/DBTSDQPInterface/user/getCaptcha';
        var INIT_URL = vge.sdqp + '/DBTSDQPInterface/sweep/getFailCount';
        var SEND_URL = vge.sdqp + '/DBTSDQPInterface/sweep/serialCode';
        var APPID = vge.sdqpappid;
        var PROJECT = 'sdqp-FIFA';
        var PHONENUM = '15311695989'; //活动未开始电话
                    ...
                    if (jo.result.code === '0') {
                        if (jo.reply) {
                            ...
                            if (jo.reply.activityVersion == '1') {
                                PROJECT = 'sdqp';
                            } else if (jo.reply.activityVersion == '2' || jo.reply.activityVersion == '3' || jo.reply.activityVersion == '4' || jo.reply.activityVersion == '5') {
                                PROJECT = 'sdqp-common';
                        ...
                        switch (jo.result.businessCode) {
                            case '0':
                                sessionStorage.weekSignFlag = jo.reply.weekSignFlag === undefined ? '' : jo.reply.weekSignFlag; //是否开户自然周签到，1:开启、0或空:关闭
                                sessionStorage.weekSignDays = jo.reply.weekSignDays === undefined ? '' : jo.reply.weekSignDays; //当前周已签到周几集合
                                sessionStorage.weekSignEarnFlag = jo.reply.weekSignEarnFlag === undefined ? '' : jo.reply.weekSignEarnFlag; //周签到红包是否已领取，1:已领取、0未领取
                                sessionStorage.weekSignEarnMoney = jo.reply.weekSignEarnMoney === undefined ? '' : jo.reply.weekSignEarnMoney; //周签到红包金额
                                sessionStorage.weekSignLimitDay = jo.reply.weekSignLimitDay === undefined ? '' : jo.reply.weekSignLimitDay; //周签到天数限制
                                sessionStorage.weekSignDiffDay = jo.reply.weekSignDiffDay === undefined ? '' : jo.reply.weekSignDiffDay; //周签到还差天数
                                sessionStorage.weekSignPercent = jo.reply.weekSignPercent === undefined ? '' : jo.reply.weekSignPercent; //进度百分比
                                sessionStorage.weekSignPopup = jo.reply.weekSignPopup === undefined ? '' : jo.reply.weekSignPopup; //自然周签到弹出提示
