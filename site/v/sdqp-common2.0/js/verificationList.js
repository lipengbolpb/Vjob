'use strict';
(function(){
	var API_findCheckUser = vge.sdqp + '/DBTSDQPInterface/checkUser/queryCheckPrizeList';//查询兑奖列表
		...
		$('.details').on('click',function(){
			for(var i=0;i<checkPrizeList.length;i++){
				if($(this).attr('infoKey')==checkPrizeList[i].infoKey){
					...
					var exchangeDate = checkPrizeList[i].exchangeDate;
					if(exchangeDate){
						$('.endTime>span').html('兑奖日期：')
						$('#endTime').val(checkPrizeList[i].exchangeDate.split(' ')[0]);
					}else{
