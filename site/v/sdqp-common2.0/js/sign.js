...
var imgUrl = '/v/sdqp-common2.0/';
var _host = vge.sdqpimg + '/DBTSDQPPlatform';
var iconFlag = true;
...
function sign() { //混合签到功能
    var tpl_1 = '<div class="swiper">' +
        ...
        '<img src="/v/sdqp-common2.0/img/sign/hb.png" class="signHb" style="display: block;">' +
        '</div>';
    var tpl_2 = '<div class="swiper">' +
        '<p class="signTotal" style="display:{{display}};">本次活动，红包总计{{total}}个，先到先得</p>' +
        '<div class="more" style="display: block;">' +
        '<p class="blessTxt">恭喜你获得</p>' +
        '<span>¥</span>' +
        '<span>{{money}}</span>' +
        '<span>元</span>' +
        '</div>' +
    ...
    if (weekSignFlag === '1') { //开启签到功能，则显示开关按钮
        ...
        if (signCogAry[0].activityFinish == '1') { //已截止
            ...
            // if (signCogAry[0].signEarnMoney == '6.66') {
            //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-1.png?v=1');
            // }else if(signCogAry[0].signEarnMoney == '7.77'){
            //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-2.png?v=1');
            // }else if(signCogAry[0].signEarnMoney == '8.88'){
            //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-3.png?v=1');
            // }else if(signCogAry[0].signEarnMoney == '9.99'){
            //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-4.png?v=1');
            // }else if(signCogAry[0].signEarnMoney == '99.99'){
            //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-5.png?v=1');
            // }else{
            ...
            // 特殊金额 
            if (signCogAry[0].signEarnMoney == '6.66') {
                $('.blessTxt').html('恭喜你获得</br>国顺家顺事事顺红包');
            }else if(signCogAry[0].signEarnMoney == '7.77'){
                $('.blessTxt').html('恭喜你获得</br>国起家起代代兴红包');
            }else if(signCogAry[0].signEarnMoney == '8.88'){
                $('.blessTxt').html('恭喜你获得</br>国富家富发发发红包');
            }else if(signCogAry[0].signEarnMoney == '9.99'){
                $('.blessTxt').html('恭喜你获得</br>国久家久运长久红包');
            }
            $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/mask.png?v=1');
        ...
        function slide(index) {
            ...
            if (signCogAry[index].activityFinish == '1') { //已截止
                ...
                // 特殊金额  你已签到成功 => 换成各种别的祝福图片
                // if (signCogAry[index].signEarnMoney == '6.66') {
                //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-1.png?v=1');
                // }else if(signCogAry[index].signEarnMoney == '7.77'){
                //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-2.png?v=1');
                // }else if(signCogAry[index].signEarnMoney == '8.88'){
                //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-3.png?v=1');
                // }else if(signCogAry[index].signEarnMoney == '9.99'){
                //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-4.png?v=1');
                // }else if(signCogAry[index].signEarnMoney == '99.99'){
                //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/sign-5.png?v=1');
                // }else{
                //     $('.signMask').find('img').attr('src', '/v/' + PROJECT + '/img/sign/mask.png?v=1');
                // }
