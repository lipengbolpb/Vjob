(function() {
	...
	var GiftsList_PORT = vge.sdqp + '/DBTSDQPInterface/gifts/queryPrizeList'; //大奖列表
	var API_getCaptcha = vge.sdqp + '/DBTSDQPInterface/user/getCaptcha'; //获取验证码接口
	var Port_Sub = vge.sdqp + '/DBTSDQPInterface/user/savePrize'; //大奖提交接口
	...
	if (location.href.indexOf('/sdqp-common2.0/txo/mybag') != -1) { //扫码打开
			...
			if (jo.result.code === '0') {
				if (jo.result.businessCode === '0') {
					if (currentpage == 1) {
						...
						$('#freeze_money').html(Number(jo.reply.freezeMoney).toFixed(2));//冻结金额
						if (dom_total < 1) {
			...
			function(jo) {
				...
				if (jo.result.code == '0') {
					if (jo.result.businessCode === '0') {
						...
						if(new Date().getTime()<new Date(2021,0).getTime()&&new Date().getTime()>new Date(2020,3,28)){//上线时间：2020年4月28日 下线时间：2020年12月31日
							$('div.tomall').css('display','flex');
							_hmt.push(['_trackEvent', 'click', '提现成功', '提现成功显示电商广告弹窗']);
						}
	...
	dom_qrbtn.addEventListener('click', function(ev) {
		...
		canvas.width = size;
		canvas.height = size;
		qrcode.makeCode('HTTP://VJ1.TV/D/'+$('#qrcode').val());
		...
		vge.ajxget(requrl, 5000, function(r) {
			try {
				...
				if (o.subscribe == '0') { //未关注
					window.location.replace('http://' + location.host + '/v/sdqp-common2.0/attention.html');
				} else { //已关注用户
					loading('提现中');
					give_spack(needAuto);
				}
