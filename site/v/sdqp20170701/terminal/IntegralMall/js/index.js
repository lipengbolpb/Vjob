...
//		$('.chose_mark').css('display','block');
	...
	function callback_loadIndex(index) {
		...
		picArr = picUrl.split(',');
//		$('.pic_box').append(`<li><img src="${vge.terminalimg}/DBTVMTSPlatform${picArr[picArr.length-1]}" /></li>`);//从后台上传
		...
		function showGoods(jo){
			...
			for(i = 0; i <l; i++) {
				html += `<dl goodsId="${jo[i].goodsId}" exchangeType="${jo[i].exchangeType}">
						<dt><img src="${vge.terminalimg}/DBTVMTSPlatform${jo[i].goodsUrl.split(',')[0]}"/></dt>
						<dd>
