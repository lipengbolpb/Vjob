(function(){
	...
	function getCheckCode(cb) { // 获取手机验证码
		var javai = vge.fjlbf + '/DBTFJLBInterface/user/getCaptcha';
		var req = {
			"phonenum":$('#tel').val(),
			"sendtype":"1"
		};
   	...
   	function sub_message () { // 提交信息
   		var javai = vge.fjlbf + '/DBTFJLBInterface/user/updateUserInfoMobile';
		var req = {
			"openid":openid,
			"phonenum":$('#tel').val(),
			"captcha":$('#yz_code').val()
    ...
    function cb(jo) {
		if(jo.result.code == '0'){
	        ...
	        case '0':               // 普通奖
				...
				location.replace('http://' + location.host + '/fjlbf/txo/getcash?bizcode='+jo.result.businessCode);
	            break;
	        case '11':              // 自己重复扫，普通奖
				sessionStorage.totalAccountMoney = jo.reply.totalAccountMoney;
				sessionStorage.currentMoney = jo.reply.currentMoney;
				sessionStorage.codeContentUrl = jo.reply.codeContentUrl;
				sessionStorage.earnTime = jo.reply.earnTime;
				location.replace('http://' + location.host + '/fjlbf/txo/repcash?bizcode='+jo.result.businessCode);
	            break;
	        case '12':              // 可疑用户
