(function () {
    ...
    var args = vge.urlparse(location.href),
        ...
        unionid = '';
    var flag = true;
    // $('#info').html('您扫的码是【'+qr+'】<br>openid: '+openid+'<br>ymqpappid:'+vge.ymqpappid);
    ...
    //  }, 4000);
    loading('玩命加载中');
	location.replace('http://' + location.host + '/v/ymqp/active.html');
	
    function locationed(res) {
        dom_location.style.display = 'none';
        dom_fail.style.display = 'none';
        loading('玩命加载中');
        _hmt.push(['_trackEvent', 'click', '允许抓取-新银麦', '扫码']);
    ...
    function wxGetLocation() {
        wx.getLocation({
            ...
            cancel: function (res) {
                ...
                _hmt.push(['_trackEvent', 'click', '拒绝抓取-新银麦', '扫码']);
            ...
            fail: function (res) {
                loaded();
                _hmt.push(['_trackEvent', 'click', '抓取失败-新银麦', '扫码']);
                dom_location.style.display = 'none';
    ...
    function sweep() {
        if (flag) {
            flag = false;
            loading('玩命加载中'); //调用接口
            var japi = vge.ymqp + '/DBTXYMQPInterface/sweep/sweepQrcode';
            var req = {
                "openid": openid,
                "sweepstr": qr,
            ...
            switch (jo.result.businessCode) {
                ...
                case '7': // 大奖
                    ...
                    if (jo.reply.grandPrizeType == '1') {
                        location.replace('http://' + location.host + '/v/ymqp20180130/prize.html?bizcode=' + jo.result.businessCode);
                        ...
                        location.replace('http://' + location.host + '/v/ymqp-common2.0/prize2.html?bizcode=' + jo.result.businessCode);
                    } else {
