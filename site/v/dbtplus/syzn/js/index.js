$(function(){
    function showList(elem,listElement, opacity){
		var elemParent = $(elem).parent();
		var listElement = elemParent.find(listElement);
		var opacityElement = elemParent.find(opacity);
		var isSet = false;
		elemParent.hover(function(){
			if(!isSet){
				var h = listElement.outerHeight();
				var w = listElement.outerWidth();
				opacityElement.css({
					width : w,
					height : h,
					opacity : 0.9
			...
			listElement.show();
			$(this).find(elem).addClass("dbt_menu_solutionHover");
		}, function(){
			listElement.hide();
			opacityElement.hide();
			$(this).find(elem).removeClass("dbt_menu_solutionHover");
		});
	}
	showList(".dbt_menu_solution", ".dbt_menu_solutionList", ".dbt_opacity");
	showList(".dbt_menu_solution1", ".dbt_menu_solutionList1", ".dbt_opacity1");
	/*var isSet = false;
