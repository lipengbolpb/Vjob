function public_tip (msg,cb) {
   var bingtoptipid = 'bingtoptip';
	...
	tipbtn.addEventListener('touchstart',function () {
...
function title_tip (tit,con,btn,jump,cb) {
	...
	alert_pub.style.cssText='width: 86%; background: #fff; border-radius: 6px; text-align: center; position: fixed; top: 26%; left: 7%; z-index: 1100; display: none; padding-bottom: 0';
	...
	if (btn===undefined||btn==='') {
		...
		tip_btn_pub.addEventListener('touchstart',function () {
...
function chose_tip (tit,con,btn_left,url_left,btn_rgh,url_rgh,cb) {
	...
	mask_pub.style.display='block';
	alert_pub.style.display='block';
	chose_btnleft.addEventListener('touchstart',function () {
	...
	chose_btnrgh.addEventListener('touchstart',function () {
