...
(function(){
	...
	$('.name').html(nickname);
	$('.money').html((sessionStorage.currentMoney/100).toFixed(2));//现金红包
			...
			if (jo.result.code == '0') {
				if (jo.result.businessCode === '0') {
					...
					if(jo.reply.userPrizeList.length != 0){//大奖列表
						...
						Vlq.style.background = "url(http://"+vge.zzqp_host +"/v/zzqpNew/img/zj/lq.png) no-repeat center/contain";
					}else{
...
//  								_hmt.push(['_trackEvent', 'click', '领取奖品', '我的']);
    							...
    							if(Vwlq[t].getAttribute('prizeType') == '3'){
    								...
    								Vwlq[t].addEventListener('click',function(){
    									location.href = 'http://vd.vjifen.com/v/zzqpNew/ticket.html?a=1&ticketCode='+this.getAttribute('ticketCode')+'&datedTime=2017-05-11';
    								},false);
    							}
    						}
	...
	//领取实物奖
	function give_jp(){
		_hmt.push(['_trackEvent', '点击领取', '实物奖', '抽奖结果']);
			...
			try{
				...
				if(o.subscribe==0) {//未关注
					...
					$('.attention').on('click',function(){
						_hmt.push(['_trackEvent', '点击关注', '关注', '抽奖结果']);
						window.location.href = 'http://'+vge.zzqp_host +'/v/zzqpNew/attention.html';//关注页面
					});
				}else{//已关注用户
        	        give_jp();
				}
	...
	function lqjp(){//显示大奖列表
		_hmt.push(['_trackEvent', '点击领取', '奖品列表', '抽奖结果']);
		$('#fyb2').fadeIn();
	...
	function jwxx(){//酒王信息提交
		_hmt.push(['_trackEvent', '点击提交', '酒王信息提交', '抽奖结果']);
...
　　　　	jpimg.src = "/v/zzqpNew/img/zj/jp"+x+".png";
		$('#jplb').append(jpimg);
		counts++;
	}
	...
	function zj(x){
		$('#cj .gz2 img').attr('src','/v/zzqpNew/img/guandao2/gz2_'+x+'.png?v=1.0.1');
		//$('#zj .jp1_1').attr('src','/v/zzqpNew/img/guandao2/gz2_'+x+'.png?v=1.0.1');
		$('#zj .jp1_2').attr('src','/v/zzqpNew/img/guandao2/zj1_'+x+'.png?v=1.0.1');
		
		if(x==0){$('.money').show();}
		if(x==4){
			$('.myjp,.top,.ck,.ck2').hide();
			$('.cq').show();
			$('.jp1_2').css('top','13%');
		}else if(x==3){
			$('.jp1_2').css('top','13%');
	...
	$('.cq').on('click',function(){
		_hmt.push(['_trackEvent', '点击去猜拳', '猜拳', '抽奖结果']);
		if(sessionStorage.cq=='1'){//猜拳胜利
			$('#comTips').css('display','block');
        	$('#fail').css('display','block');
			$('#tip').html('您已经赢了一把<br />再扫一罐吧!');
		}else if(sessionStorage.cq=='2'){
			$('#comTips').css('display','block');
        	$('#fail').css('display','block');
			$('#tip').html('您已经输了一把<br />再扫一罐吧!');
		}else{
			location.href = '/v/zzqpNew/g3/index.html';
		}		
	});
		
	
	function give_spack() {//提现
		_hmt.push(['_trackEvent', '点击提现', '提现', '抽奖结果']);
		Vtx.removeEventListener('click',give_spack,false);
//		$('#comTips').css('display','block');
//  	$('#fail').css('display','block');
//		$('#tip').html('微信提现升级中，您可先扫码，红包会自动累积哦！');
	...
	$('#toMyJp').on('click',function(){
		...
		location.href = 'http://'+location.host+'/v/zzqpNew/myJp.html';
	});
