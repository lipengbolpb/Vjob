(function() {
	$(document).ready(function() {
		let CODE_URL = vge.sxqp + '/DBTSXQPInterface/user/getCaptcha';
		let INIT_URL = vge.sxqp + '/DBTSXQPInterface/sweep/getFailCount';
		let SEND_URL = vge.sxqp + '/DBTSXQPInterface/sweep/serialCode';
		let APPID = vge.sxqpappid;
		let PROJECT = 'sxqp-common';
				...
				if(jo.result.code === '0') {
					...
					switch(jo.result.businessCode) {
						...
						case '4': // 4 - 活动未开始
							...
							$('.title').html('活动未开始');
							$('.content').html('亲再等等哦~');
							$('.batch').html(jo.reply.batchName + '<br />服务热线：15311695989');
							break;
						case '5': // 5 - 活动已结束
							$('.slogan').css('display', 'none');
							$('.mask').css('display', 'block');
