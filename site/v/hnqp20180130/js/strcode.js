$(document).ready(function () {
    let CODE_URL = vge.hnqp + '/DBTHuaNQPInterface/user/getCaptcha';
    let INIT_URL = vge.hnqp + '/DBTHuaNQPInterface/sweep/getFailCount';
    let SEND_URL = vge.hnqp + '/DBTHuaNQPInterface/sweep/serialCode';
    let APPID = vge.hnqpappid;
    let PROJECT = 'hnqp20180130';
			...
			if(jo.reply.activityVersion == '3') {
				...
				switch(jo.result.businessCode) {
					case '0': // 普通奖
						...
						location.replace('http://' + location.host + '/sxqp20180130/txo/getcash?bizcode=' + jo.result.businessCode);
						break;
					case '11': // 自己重复扫，普通奖
						sessionStorage.totalAccountMoney = jo.reply.totalAccountMoney;
						sessionStorage.currentMoney = jo.reply.currentMoney;
						...
						break;
					case '12': // 可疑用户
						location.replace('http://' + location.host + '/v/sxqp20180130/getMsg.html?bizcode=' + jo.result.businessCode);
						break;
					case '13': // 黑名单
						location.replace('http://' + location.host + '/v/sxqp20180130/getMsg.html?bizcode=' + jo.result.businessCode);
						break;
					...
					default:
						...
						location.replace('http://' + location.host + '/v/sxqp20180130/fail.html?bizcode=' + jo.result.businessCode);
				}
			} else {
				sessionStorage.skukey = jo.reply.skukey;
				sessionStorage.activityVersion = jo.reply.activityVersion;
				switch(jo.result.businessCode) {
					case '0': // 普通奖
						...
						sessionStorage.weekSignPopup = jo.reply.weekSignPopup; //自然周签到弹出提示
						sessionStorage.earnTime = jo.reply.earnTime;
						location.replace('http://' + location.host + '/sxqp/txo/result?bizcode=' + jo.result.businessCode);
						break;
					case '11': // 自己重复扫，普通奖
						sessionStorage.totalAccountMoney = jo.reply.totalAccountMoney;
						sessionStorage.currentMoney = jo.reply.currentMoney;
						...
						break;
					case '12': // 可疑用户
						location.replace('http://' + location.host + '/v/sxqp/getMsg.html?bizcode=' + jo.result.businessCode);
						break;
					case '13': // 黑名单
						location.replace('http://' + location.host + '/v/sxqp/getMsg.html?bizcode=' + jo.result.businessCode);
						break;
					...
					default:
						...
						location.replace('http://' + location.host + '/v/sxqp/fail.html?bizcode=' + jo.result.businessCode);
				}
			}
