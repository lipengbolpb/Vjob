(function() {
	$('#content').load('class.html #class', function() {
			var jo = [],
			i = 0,
			html = '';
			if(sessionStorage.classList){
				...
				for(i = 0; i < jo.length; i++) {//style="background-image:url(${jo[i].categoryImg})"
					html += ` <div class="div${jo[i].categoryType}"><span class="name">${jo[i].categoryName}</span></div>`;
				}

				$('#class').append(html);
				$('#class').on('click', 'div', function() {
					sessionStorage.classname = $(this).attr('class')[3];
					sessionStorage.categoryParent = $(this).attr('class')[3];
			...
			function getFistCategoryType() { //获取所有一级品类
				var javai = vge.zzqpWeChat + '/DBTHNQPInterface/vpointsShop/getFistCategoryType';
				var req = {};
