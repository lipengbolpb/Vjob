(function () {
    var args = vge.urlparse(decodeURI(location.href)),
        openid = args.openid,
    ...
    if (nickname) {
        // 先检测连接
        nickname = nickname;
        // sessionStorage.nickname = nickname == undefined ? 'undefined' : nickname;
    }else if(sessionStorage.nickname){
        // 再检测缓存
    ...
    if (headimgurl) {
        ...
        // sessionStorage.headimgurl = headimgurl == undefined ? 'undefined' : headimgurl;
    } else if(sessionStorage.headimgurl){
        headimgurl = headimgurl;
    }else{
    ...
    function callback_loadIndex(index) {
        ...
        var picUrl = 'img/banner_21.jpg,img/banner_22.jpg,img/banner_23.jpg', //picUrl = jo.goodsUrl,
            picArr = [];
        ...
        $('#game_box .game').eq(0).on('click', 'dt,a', function () {
            // title_tip('尊敬的用户', '敬请期待！', '我知道了');
            // window.location.href = 'https://' + location.host + '/zzqp20171214/to/footBall';
            window.location.href = 'https://' + location.host + '/v/zzqpWeChat/footBall/index.html';
        });
        $('#game_box .game').eq(1).on('click', 'dt,a', function () {
            // window.location.href = 'https://' + location.host + '/v/zzqp20171214/game1/index.html';
            window.location.href = 'https://' + location.host + '/v/zzqpWeChat/game1/index.html';
            // title_tip('尊敬的用户', '敬请期待！', '我知道了');
        });
        ...
        function getShopGoods() { //获取商品数据
            var javai = vge.zzqpWeChat + '/DBTHNQPInterface/vpointsShop/getShopGoods';
            var req = {
                "currentPage": 1,
                "count": 10,
        ...
        function userInfo() { //用户积分商城个人信息 
            var javai = vge.zzqpWeChat + '/DBTHNQPInterface/vpointsExchange/userVpointsShopInfo';
            var req = {
                "openid": openid,
                "companyKey": sessionStorage.companyKey
            };
        ...
        function showGoods(jo) {
            ...
            for (i = 0; i < l; i++) {
                html += `<dl goodsId="${jo[i].goodsId}" exchangeType="${jo[i].exchangeType}">
						<dt><img src="${vge.zzqpWeChatimg}/DBTHNQPPlatform${jo[i].goodsUrl.split(',')[0]}"/></dt>
						<dd>`;
    ...
    function getAllVpointsShops() { //获取商城积分数据
        var javai = vge.zzqpWeChat + '/DBTHNQPInterface/vpointsShop/getAllVpointsShops';
        var req = {
            "openid": openid
        };
        vge.callJApi(javai, req,
