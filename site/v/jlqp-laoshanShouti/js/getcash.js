(function () {
    ...
    if (totalAccountMoney < 1) {
        $('#btn').css({
            'background': 'url(/v/jlqp-laoshanShouti/img/button_tobag.png?v=1) no-repeat center',
            '-webkit-background-size': 'auto 100%'
        });
        if (currentMoney == 0) {
            $('#btn').css({
                'background': 'url(/v/jlqp-laoshanShouti/img/button_look.png?v=1) no-repeat center',
                '-webkit-background-size': 'auto 100%'
            });
        ...
        $('#btn').css({
            'background': 'url(/v/jlqp-laoshanShouti/img/button_tx.png?v=1) no-repeat center',
            '-webkit-background-size': 'auto 100%'
        });
    ...
    function subscribe() { //判断关注
        ...
        vge.ajxget(requrl, 5000, function (r) {
            try {
                var o = JSON.parse(r);
                vge.clog('关注结果', [requrl, r]);
                if (o.subscribe == 0) { //未关注
                    window.location.replace('http://' + location.host + '/v/jlqp-laoshanShouti/attention.html');
                } else { //已关注用户
                    window.location.replace('http://' + location.host + '/jlqp-laoshanShouti/too/mybag');
                }
            } catch (e) {
    ...
    $('.rule').on('click', function () {
        location.href = "https://mp.weixin.qq.com/s?__biz=MzUxNDg5OTU3Nw==&mid=100000783&idx=1&sn=ca65f43cfca7d5491939aebc27cd37ad&chksm=79bfaf194ec8260f5fedf965fffa15bbd76b2492378510fa4d51a00489220140bc603b953ee2#rd";
    });
