var res = JSON.parse(sessionStorage.jo),
...
function base(flag) {
    if (res) {
        if (flag) { //箱码
            var dom_tr = document.querySelector('.basic_box').querySelectorAll('tr');
...
function trend(flag) { //7天扫码趋势表
    ...
    for (var i = 0; i < l; i++) {
        if (flag) { //箱码
            ...
            $('.legend').find('p').eq(0).html('<span></span>消费箱数');
            $('.legend').find('p').eq(3).html('<span></span>中出积分');
        } else { //盖码
            /**
            ...
            $('.legend').find('p').eq(0).html('<span></span>消费瓶数');
            $('.legend').find('p').eq(3).html('<span></span>参与人数');
        }
    }
...
function tool(i, flag) {
    // 右上角详情部分
    // 日期
    var nowDate = res.sevenDayList[i].reportDate.substr(0, 2) + '月' + res.sevenDayList[i].reportDate.substr(3, 2) + '日';
    if (flag) {
        $('#tool_date').html(nowDate + '&nbsp;&nbsp;' + res.sevenDayList[i].weekDay);
        // 消费箱数
        $('#tool_box').html('<span></span>消费箱数 / 箱：' + res.sevenDayList[i].scanCount + '(' + res.sevenDayList[i].scanCountP + ')');
        // 消费金额
        $('#tool_money').html('<span></span>消费金额 / 元：' + res.sevenDayList[i].moneyCount + '(' + res.sevenDayList[i].moneyCountP + ')');
        // 终端数量
        $('#tool_num').html('<span></span>参与终端 / 个：' + res.sevenDayList[i].terminalCount + '(' + res.sevenDayList[i].terminalCountP + ')');
        // 中出积分
        $('#tool_point').html('<span></span>中出积分 / 分：' + res.sevenDayList[i].vpointsCount + '(' + res.sevenDayList[i].vpointsCountP + ')');
    } else {
        $('#tool_date').html(nowDate + '&nbsp;&nbsp;' + res.sevenDayList[i].weekDay);
        // 消费箱数
        $('#tool_box').html('<span></span>消费瓶数 / 瓶：' + res.sevenDayList[i].scanCount + '(' + res.sevenDayList[i].scanCountP + ')');
        // 消费金额
        $('#tool_money').html('<span></span>消费金额 / 元：' + res.sevenDayList[i].moneyCount + '(' + res.sevenDayList[i].moneyCountP + ')');
        // 终端数量
        $('#tool_num').html('<span></span>参与终端 / 个：' + res.sevenDayList[i].terminalCount + '(' + res.sevenDayList[i].terminalCountP + ')');
        // 中出积分
        $('#tool_point').html('<span></span>参与人数 / 人：' + res.sevenDayList[i].userCount + '(' + res.sevenDayList[i].userCountP + ')');
    }
}
...
function detail(flag) { //明细表格
    $('.sub').empty();
    // var nowDate = queryDate.substr(5, 2) + '月' + queryDate.substr(8, 2) + '日';
    if (flag) {
        $('.subTitle').text('二级区域当日明细');
        $('.sub').append('<tr>' +
            '<td>排名</td>' +
            '<td>二级区域</td>' +
            '<td>消费箱数（箱）</td>' +
    ...
    if (res) {
        ...
        if (flag) {
            ...
            $('.sub').append(tplLast);
            $('.limitVpoints').text(res.limitVpoints);
            $('.limitDay').text(res.limitDay);
        } else {
            for (var i = 0; i < l; i++) {
                var tpl = template.replace('{{num}}', i + 1)
                    .replace('{{county}}', res.countyList[i].county)
...
$('#btn_box').on('click', function () { //显示箱码
    loading('报表加载中');
    $('#btn_box').css({
    ...
    var requrl = 'http://59.110.53.118:9008/DBTMainEntStats/reportDetail/getZDCXData.do?queryDate=' + queryDate + '&type=1&company=' + company;
    $.ajax({
        async: true,
        type: "GET",
        url: requrl,
        success: function (jo) {
            ...
            if (res.skuList) {
                skuList();
                $('.skulist,.skulistTitle').removeClass('hidden');
            } else {
                $('.skulist,.skulistTitle').addClass('hidden');
            }
            $('.basic_lid').addClass('hidden'); //隐藏箱码
            $('.basic_box').removeClass('hidden'); //显示盖码
        },
        error: function () {
...
$('#btn_lid').on('click', function () { //显示盖码
    loading('报表加载中');
    $('#btn_box').css({
    ...
    var requrl = 'http://59.110.53.118:9008/DBTMainEntStats/reportDetail/getZDCXData.do?queryDate=' + queryDate + '&type=2&company=' + company;
    $.ajax({
        async: true,
        type: "GET",
        url: requrl,
        success: function (jo) {
            ...
            $('.basic_box').addClass('hidden'); //隐藏箱码
            $('.basic_lid').removeClass('hidden'); //显示盖码
        },
        error: function () {
            title_tip('提 示', '系统开了个小差，请稍后重试！', '我知道了');
        }
