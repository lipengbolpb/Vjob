$(document).ready(function () {
    ...
    // doublePrize.doublePrizeLottery = '1';
    // doublePrize.doublePrizeFlag = '1';
    /**
    ...
    $('.doubleAlertNo').one('click', function () {
        _hmt.push(['_trackEvent', 'click', '一码双奖', '放弃抽奖']);
        var req = {
    ...
    $('.doubleAlertYes').one('click', function () {
        _hmt.push(['_trackEvent', 'click', '一码双奖', '点击抽奖']);
        var req = {
            "openid": sessionStorage.openid,
            "vcodeActivityKey": doublePrize.doublePrizeActivityKey,
        ...
        $.ajax({
            ...
            success: function (jo, status, xhr) {
                if (jo.result.code == '0') {
                    if (jo.result.businessCode === '0') {
						vge.clog('抽奖信息', [JSON.stringify(jo)]);
                        ...
                        if (jo.reply.prizeImg.indexOf('R.png') != -1 || jo.reply.prizeImg.indexOf('r.png') != -1) {
                            $('.doubleGetMain').css('display', 'none');
                            $('.doubleGetYYT').css('display', 'block');
                            $('.doubleGet').show();
                            $('.doubleAlert').hide();
                        } else {
    ...
    function initList(list, flag, cb) { //显示列表
        ...
        var tpl1 = '<li>' +
            ...
            '<img src="/v/hbqp-common3.0/img/double/prize-line.png">' +
        ...
        var tpl2 = '<li>' +
            ...
            '<div><p>{{prizeName}}</p><p>中奖时间：<span>{{earnTime}}</span></p><p>领奖截止：<span>{{expireTime}}</span></p></div>' +
            '<img src="/v/hbqp-common3.0/img/double/prize-line.png">' +
            '<img src="/v/hbqp-common3.0/img/double/getted.png">' +
            '</li>';
        // 已领取
        var tpl3 = '<li>' +
            '<img src="{{prizeImg}}">' +
        ...
        if (flag == true) { //未领取界面
            ...
            if ($('.doubleListDetailed>li').length >= 1) {
                ...
                $('.doubleListNone').attr('src', '/v/hbqp-common3.0/img/double/list-empty.png');
            ...
            if ($('.doubleListDetailed>li').length >= 1) {
                ...
                $('.doubleListNone').attr('src', '/v/hbqp-common3.0/img/double/list-none.png');
                $('.doubleListNone').css('display', 'block');
                $('.doubleListDetailed').css('display', 'none');
            }
        }
