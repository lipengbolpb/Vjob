(function () {
    ...
    var RULE_HREF = 'https://mp.weixin.qq.com/s?__biz=MzIzMTA1NzgzNw==&mid=100000108&idx=1&sn=2248ff349c04544763581be71071eccf&chksm=68a8b1ae5fdf38b8c06ff399672bc69b0a7660d8d00904f57697c931c3579ad77212c98b1813#rd';
    var SECRET_HREF = 'https://mp.weixin.qq.com/s?__biz=MzIzMTA1NzgzNw==&mid=100000110&idx=1&sn=94c428a4aaf4c59184c737c89087c080&chksm=68a8b1ac5fdf38ba95c01fac98c7f9216f36c5c72f256d0a77030d2b8650bded39bfa3e276f6#rd';
    ...
    $(document).ready(function () {
        if (grandPrizeType == 's' || grandPrizeType == 'S' || grandPrizeType == 't' || grandPrizeType == 'T') {
            init();
        }
        ...
        function init() {
            if (bizcode == 15) {
                if (grandPrizeType == 's' || grandPrizeType == 'S') {
                    $('.sloganRep').attr('src', '/v/fjqp20181120/img/prize-slogan-1.png');
                } else if (grandPrizeType == 't' || grandPrizeType == 'T') {
                    $('.sloganRep').attr('src', '/v/fjqp20181120/img/prize-slogan-2.png');
                    $('.sloganRep').css('width','35%');
                }
                ...
                // s 手表 t 耳机
                if (grandPrizeType == 's' || grandPrizeType == 'S') {
                    $('.prizeTypeGet').attr('src', '/v/fjqp20181120/img/prize-slogan-1.png');
                    $('.prizeTypeInfo').attr('src', '/v/fjqp20181120/img/prize-1.png');
                    $('.prizeLv').html('一等奖');
                    $('.prizeName').html('一等奖');
                    $('.prizeInfo').html('华为第二代智能运动手表4G版1个<br>价值1688元，共200个');
                    $('.prizeTypeGet').css('width', '70%');
                } else if (grandPrizeType == 't' || grandPrizeType == 'T') {
                    $('.prizeTypeGet').attr('src', '/v/fjqp20181120/img/prize-slogan-2.png');
                    $('.prizeTypeInfo').attr('src', '/v/fjqp20181120/img/prize-2.png');
                    $('.prizeLv').html('二等奖');
                    $('.prizeName').html('二等奖');
                    $('.prizeInfo').html('小米蓝牙耳机1个，价值169元<br>共2000个');
                    $('.prizeTypeGet').css('marginLeft', '33%');
                    $('.prizeTypeInfo').css('marginLeft', '38%');
                }
        ...
        $('#btn').on('click', function () {
            if (!$('.name').val() == '' && !$('.name').val().indexOf(' ') !== -1 && !$('.address').val() == '' && !$('.address').val().indexOf(' ') !== -1 && reg1.test($('.tel').val()) && reg3.test($('.code').val())) {
                send();
            }
        });
        ...
        function getYzcode() {
            if ($('.name').val() === '' || $('.name').val().indexOf(' ') !== -1) {
                ...
                title_tip('提 示', '请输入正确的邮寄地址哦！~', '我知道了');
