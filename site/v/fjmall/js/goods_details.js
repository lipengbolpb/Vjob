...
(function() {
	...
	var goodsHtml = '',
		jo = JSON.parse(sessionStorage.jo).concat(JSON.parse(sessionStorage.jo_type_index
		), JSON.parse(sessionStorage.recommendGoods)),
	...
	function showDetails(){//展示单个商品
		for (var i = 0; i < jo.length; i++) {
			// console.log(jo[i].goodsId, sessionStorage.goodsId);
			...
			if (jo[i].goodsId == sessionStorage.goodsId) {
				...
				goodsHtml =
					`<img src="${handleImgUrl(jo[i].goodsUrl, 1)}" class="goods"/>
				<div class="goods_msg">
					<p class="goodsname">${jo[i].goodsName}</p>
					<div>
						<p class="price">积分：<span>${transfNum(jo[i].realVpoints)}</span></p>
						<p class="scprice">市场价值：<span class="scprice-num">${jo[i].goodsMoney}元</span></p>
					</div>
					<div class="num">
						<p>数量</p>
						<div>
					...
					<div class="tip">
						<div>
							<p>正品行货</p>
							${jo[i].expressTips ? '<p class="senddate">' + jo[i].expressTips + '</p>' : ''}
							<p>不支持退货</p>
						</div>
					</div>
				</div>
				<div class="goods_info">
					<p><span class="line"></span>商品介绍<span class="line"></span></span></p>
					<div>${jo[i].goodsContent}</div>				
				</div>`;
				$('#goods_details').append(goodsHtml);
				$('#goods_details .goods_info div').find('*').attr('style', '');
				$('#order_msg-price').text(transfNum(jo[i].realVpoints))
				//加减数量
		...
		$('.add').on('click', function() {
			if ($('.addgoods1').val() - 0 + 1 > goods_details.goodsRemains) {
				...
				$('#save_box .vpoints span').html(transfNum(goods_details.realVpoints*(sessionStorage.addgoods==undefined?1:sessionStorage.addgoods)));
			}
	...
	// 兑换前不可修改地址弹窗提醒
	function beforeChange(cb) {
		$('#not-change').css('display', 'flex');
		var timerCount = 3;
		$('#not-change .btn-default').removeClass('active').text(`我已确认(${timerCount})`);

		var timer = setInterval(function () {
			timerCount--;
			if (timerCount < 0) {
				clearInterval(timer);
				$('#not-change .btn-default').addClass('active').text(`我已确认`);
				return
			}
			$('#not-change .btn-default').text(`我已确认(${timerCount})`)
		}, 1000);

		$('.close-img').unbind().click(function () {
			clearInterval(timer);
			$('#not-change').css('display', 'none');
		});

		$('#not-change .btn').unbind().click(function () {
			if($(this).hasClass('active')) { // 只有按钮变蓝才能有操作
				$('#not-change').css('display', 'none');
				loading('兑换中');
				cb && cb();
			}	
					...
					if (jo.reply) {
						...
						<p>${jo.reply.province+jo.reply.city+jo.reply.county+jo.reply.address}</p>`
							...
							if(sessionStorage.goodsId){//单个订单
								...
								$('#order img').attr('src', `${handleImgUrl(goods_details.goodsUrl)}`);
								$('#order .goodsname').text(goods_details.goodsName);
								// $('#order .price span').text(transfNum(goods_details.realVpoints * sessionStorage.addgoods));
								$('#save_box .vpoints span').html(transfNum(goods_details.realVpoints*sessionStorage.addgoods));
							}
						}
					} else {
	...
	$('#exchange').on('click', function(e) { //兑换商品
		...
		if(sessionStorage.goodsId){//单件商品
			...
			if(sumnum>sessionStorage.surplusVpoints){
				...
				// loading('兑换中');
				beforeChange(goodsExchangeForCart);
			}
		}
	});
			...
			function(jo) {
				...
				if (jo.result.code == '0') {
					if (jo.result.businessCode == '0') { //成功
						...
						$('#alert').fadeIn(100);
						if (jo.result.msg !== '兑换成功') {
							$('.tip-fail').css('display', 'block');
						}
					} else {
						if (jo.result.businessCode == '2') {
							title_tip('尊敬的用户', '已发起兑换，预计10分钟内充值完成！', '我知道了');
							...
							title_tip('尊敬的用户', jo.result.businessCode + ':' + jo.result.msg, '我知道了', 'index.html#/shopcart');
						}
					}
				} else {
					title_tip('尊敬的用户', jo.result.businessCode + ':' + jo.result.msg, '我知道了');
	...
	function showRecommend(jo) {
		...
		$('#alert .goods-item').click(checkGoodsDetail)
	}
