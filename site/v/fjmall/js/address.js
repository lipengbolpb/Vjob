...
//							}
							...
							$('#address_list .change').on('click', function(e) { //修改
								...
								$('#addressval').val(jo.reply[addIndex].provinceCode + '-' + jo.reply[addIndex].cityCode + '-' + jo.reply[addIndex].countyCode);
...
//							title_tip('提 示', , '我知道了');
	...
	function updateAddress() { //修改地址
		...
		var req = {
			...
			"cityCode": $('#addressval').val().split('-')[1],
			"county": $('#area').val().split('-')[2]==undefined?$('#area').val().split('-')[1]:$('#area').val().split('-')[2],
			"countyCode": $('#addressval').val().split('-')[2] == undefined ? $('#addressval').val().split('-')[1] : $('#addressval').val().split('-')[2],
			"openid": sessionStorage.openid,
			"addressId": sessionStorage.addressId,
			"phoneNum": $('#phone').val(),
			"province": $('#area').val().split('-')[0],
			"provinceCode": $('#addressval').val().split('-')[0],
			"userName": $('#user').val()
		};
		vge.callJApi(javai, req,
			function(jo) {
