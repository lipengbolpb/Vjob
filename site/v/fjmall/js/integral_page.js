...
(function() {
			...
			function(jo) {
				if(jo.result.code == '0') {
					if(jo.result.businessCode == '0') {
						if(currentPage==1){
							$('.total_get .num').html(transfNumWan(jo.reply.accountVpoints));
							$('.total_pay .num').html(transfNumWan(jo.reply.exchangeVpoints));
						}
						...
						if(le>0){
							...
							for(var j=0;j<le;j++){
								integralHtml += `<li>
									...
									<div>
										<p>+${transfNum(jo.reply.objList[j].earnVpoints)}</p>
									</div>
								</li>`;
							}
			...
			function(jo) {
				if(jo.result.code == '0') {
					if(!jo.result.businessCode) {
						...
						<div>
							<p>-${transfNum(jo[i].exchangeVpoints)}</p>
						</div>
					</li>`;
						}
	...
	function transfNumWan(num) {
		num = num * 1;
		var remainder = '';
		if (num > 10000) {
			remainder = '.' + (num % 10000 + '').substr(0, 1) + '万';
			num = Math.floor(num / 10000);
			
		}
		return transfNum(num) + remainder;
	}
