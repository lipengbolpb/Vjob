(function(){
    ...
    function only_strcode () {
        ...
        str_code.addEventListener('keyup',function () {
            if (reg1.test(str_code.value)) {
				$('#btn').css({'background':'url(/v/csqp20170920/img/button_3.png) no-repeat center','-webkit-background-size':'auto 100%'});
				btn.addEventListener('click',dot,false);
            } else{
            	btn.removeEventListener('click',dot,false);
                $('#btn').css({'background':'url(/v/csqp20170920/img/button_4.png) no-repeat center','-webkit-background-size':'auto 100%'});
            }
        },false);
    }
    ...
    function init () {
        var javai = vge.csqp + '/DBTHuNanQPInterface/sweep/getFailCount';
        var req = {
            "openid":openid
        };
        vge.callJApi(javai, req, function(jo) {
    ...
    function sendcode() {
        var javai = vge.csqp + '/DBTHuNanQPInterface/sweep/serialCode';
        var req = {
            "openid":openid,
            "serialcode":scode,
        ...
        vge.callJApi(javai, req, function(jo) {
	        ...
	        case '0':               // 普通奖
				...
				location.replace('http://' + location.host + '/csqp20170920/txo/getcash?bizcode='+jo.result.businessCode);
	            break;
	        case '11':              // 自己重复扫，普通奖
				sessionStorage.totalAccountMoney = jo.reply.totalAccountMoney;
				sessionStorage.currentMoney = jo.reply.currentMoney;
				sessionStorage.codeContentUrl = jo.reply.codeContentUrl;
				sessionStorage.earnTime = jo.reply.earnTime;
				location.replace('http://' + location.host + '/csqp20170920/txo/repcash?bizcode='+jo.result.businessCode);
	            break;
	        case '12':              // 可疑用户
				location.replace('http://' + location.host + '/v/csqp20170920/getMsg.html?bizcode='+jo.result.businessCode);
	            break;    
	        case '13':              // 黑名单
				location.replace('http://' + location.host + '/v/csqp20170920/getMsg.html?bizcode='+jo.result.businessCode);
	            break;  
	        ...
	        case '-1':              // 升级中
	        	...
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_-1.png?v=1');
	            break; 
	        case '1':              // 不存在
	        ...
	        case '2':              // 被扫过
				$('.fail_tip').css('display','block');
        		$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_2.png');
	        ...
	        case '3':              // 过期
				$('.fail_tip').css('display','block');
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_3.png');
	            break; 
	        case '17':              // 过期
				$('.fail_tip').css('display','block');
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_3.png');
	        ...
	        case '4':              // 未开始
				$('.fail_tip').css('display','block');
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_4.png');
	        	$('#batch').css('display','block');
	        	$('#batch').html(jo.reply.batchName+'<br />服务热线：13011121918');
	            break;
	        case '5':              // 已截止
				$('.fail_tip').css('display','block');
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_5.png?v=1');
	            break;
	        case '6':              // 指定
				$('.fail_tip').css('display','block');
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_6.png');
	            break;  
	        case '8':              // 需要验证码
				need_yzcode();
	            break;
	        ...
	        case '10':              // 错误超过6次,请明天再试
				$('.fail_tip').css('display','block');
	        	$('.fail_msg').attr('src','/v/csqp20170920/img/fail_tip_many.png');
	            break;    
	        default:
