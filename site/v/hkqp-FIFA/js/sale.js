(function () {
    ...
    var promotionFlag = '', //是否开启1:开启、0或空:关闭
        promotionPopup = '', //1:弹出提示、0或空:不弹出
        promotionCogAry = []; //活动列表

    var PROJECT = 'hkqp-FIFA';
    var imgUrl = '/v/hkqp-FIFA/';
    var _host = vge.hkqpimg + '/DBTHaiNQPplatform';

    var timmerSale = setInterval(function () {
        promotionFlag = sessionStorage.promotionFlag == 'undefined' ? '0' : sessionStorage.promotionFlag; //是否开启1:开启、0或空:关闭
    ...
    function sale() {
        ...
        if (promotionFlag == '1') { //1为开启
            if (bizcode == 11 || again == 'true') {
                ...
                // document.getElementsByClassName('foam')[0].addEventListener('animationend', function () {
