...
//		location.href = (location.href).split('#')[0]+'#person';
	...
	function callback_loadIndex(index) {
		...
		var picUrl = 'img/banner.png',//picUrl = jo.goodsUrl,
			picArr = [];
...
//		$('.pic_box').append(`<li><img src="${vge.csqp}/DBTHuNanQPPlatform${picArr[0]}" /></li>`);
		...
		function getShopGoods() { //获取商品数据
			var javai = vge.csqp + '/DBTHuNanQPInterface/vpoints/vpointsShop/getShopGoods';
			var req = {
				"currentPage":1,
				"count":50,
		...
		function userInfo() { //用户积分商城个人信息 
			var javai = vge.csqp + '/DBTHuNanQPInterface/vpoints/vpointsExchange/userVpointsShopInfo';
			var req = {
		...
		function showGoods(jo){
			//数据填充
			var l = jo.length>50?50:jo.length;
			for(i = 0; i <l; i++) {
				html += `<div class="flex_column"><dl goodsId="${jo[i].goodsId}" exchangeType="${jo[i].exchangeType}">
						<dt><img src="${vge.csqpimg}/DBTHuNanQPPlatform${jo[i].goodsUrl.split(',')[0]}"/></dt>
						<dd>`;
				...
				if(jo[i].realPay==0){
					html += `<span><i>${jo[i].realVpoints}</i>积分</span>
						</dd>
					</dl></div>`;
				}else{
					html += `<span><i>${jo[i].realVpoints}</i>积分<i class="pay_money">+${jo[i].realPay}元</i></span>
						</dd>
					</dl></div>`;
...
//			html = `<p class="title">————大家都在兑————</p>`;
	...
	function getAllVpointsShops() { //获取商城积分数据
		var javai = vge.csqp + '/DBTHuNanQPInterface/vpoints/vpointsShop/getAllVpointsShops';
		var req = {
			"openid":sessionStorage.openid
		};
		vge.callJApi(javai, req,
