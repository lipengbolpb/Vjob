(function() {
	...
	$('#content').load('class.html #list', function() {
		...
		var isCommend=parseInt(sessionStorage.categoryParent);
		if(isNaN(parseInt(sessionStorage.categoryParent))){
			isCommend = 'first';
		}
		goodsListShow(isCommend, undefined);
		categoryList();

		function categoryList() { //获取菜单
			var javai = vge.csqp + '/DBTHuNanQPInterface/vpoints/vpointsShop/getAllCategoryType';
			var req = {};
		...
		function categoryListShow(jo) {//显示菜单列表
			...
			for(i = 0; i < jo.length; i++) {
				html += `<div class="list_box ${jo[i].categoryIco.split('-')[1]}" categoryParent='first'>
			...
			$('.list_box').each(function() {
				if($(this).attr('class').indexOf(sessionStorage.classname.split(' ')[1]) !== -1) {
					$(this).addClass('cur');
				}
			})
				...
				for(k = 0; k < goodsList.length; k++) {
					if(goodsList[k].categoryType == b) {
						htmlGoods += `<dl goodsid='${goodsList[k].goodsId}' exchangeType="${goodsList[k].exchangeType}">
								<dt><img src="${vge.csqpimg}/DBTHuNanQPPlatform${goodsList[k].goodsUrl.split(',')[0]}" class="goods_pic"/></dt>
								<dd>
									<p class="name">${goodsList[k].goodsName}</p>
									<em goodsDiscount='${goodsList[k].goodsDiscount}'>${goodsList[k].goodsDiscount/10}折优惠</em>
									<p class="price">兑换积分：<span>${goodsList[k].realVpoints}</span><del goodsDiscount='${goodsList[k].goodsDiscount}'>${goodsList[k].goodsVpoints}</del></p>
								</dd>
							</dl>`;
					}
				}
