(function(){
	...
	$('#alert').on('click',function(){
		$('.wxmark').animate({'display':'none'},1,function(){
			location.reload();
		});
	});
	...
	function attentioned () {
		...
		function onepage_note(currentpage,cb) {
			var javai = vge.csqp + '/DBTHuNanQPInterface/gifts/queryAllGiftsList';
			// alert(javai);
			var req = {
				"openid":openid,
				"hbopenid":hbopenid,
		            ...
		            if( jo.result.businessCode==='0') {
						...
						for (i = 0; i < l; ++i) {
							...
							if (mon_where==='扫码中奖'||mon_where==='签到红包') {
								...
								params.color='#ffd46b';
								params.imgUrl = 'background-image:url(/v/csqp/img/icon-code.png);';
							} else{
