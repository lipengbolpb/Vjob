(function(){
	...
	function getCheckCode(cb) { // 获取手机验证码
   	...
   	function sub_message () { // 提交信息
   		var javai = vge.csqp + '/DBTHuNanQPInterface/user/updateUserInfoMobile';
		var req = {
			"openid":openid,
			"phonenum":$('#tel').val(),
			"captcha":$('#yz_code').val()
    ...
    function cb(jo) {
		...
		if(jo.result.code == '0'){
	        ...
	        case '0':               // 普通奖
				...
				location.replace('http://' + location.host + '/csqp/txo/getcash?bizcode='+jo.result.businessCode);
	            break;
	        case '11':              // 自己重复扫，普通奖
				sessionStorage.totalAccountMoney = jo.reply.totalAccountMoney;
				sessionStorage.currentMoney = jo.reply.currentMoney;
				sessionStorage.earnTime = jo.reply.earnTime===undefined?'':jo.reply.earnTime;//扫码时间
				location.replace('http://' + location.host + '/csqp/txo/repcash?bizcode='+jo.result.businessCode);
	            break;
	        case '7':              // 大奖
				...
				location.replace('http://' + location.host + '/v/csqp/prize.html');
	            break;   
	        case '12':
	        	location.replace('http://' + location.host + '/v/csqp/getMsg.html?bizcode='+jo.result.businessCode);
	        	break;
	        case '13':
	        	location.replace('http://' + location.host + '/v/csqp/getMsg.html?bizcode='+jo.result.businessCode);
	        	break;
	        ...
	        case '15':  //他人重复扫大奖
                sessionStorage.grandPrizeType = jo.reply.grandPrizeType===undefined?'':jo.reply.grandPrizeType;
                sessionStorage.earnTime = jo.reply.earnTime===undefined?'':jo.reply.earnTime;//扫码时间
                location.replace('http://' + location.host + '/v/csqp/prize.html?bizcode='+jo.result.businessCode);
                break;	
	        default:
	        	if(jo.reply){
	        		sessionStorage.batchName = jo.reply.batchName===undefined?'':jo.reply.batchName;
	        		sessionStorage.earnTime = jo.reply.earnTime;//扫码时间
	        		sessionStorage.msg = jo.result.msg;
	        	}	
				location.replace('http://' + location.host + '/v/csqp/fail.html?bizcode='+jo.result.businessCode);
	        }	
