$(document).ready(function () {
    // 接口
    var Port_GiveUp = vge.jsqp + '/DBTJSQPInterface/doubleprize/cancelLotteryPrize'; //放弃抽奖接口
    var Port_Lottery = vge.jsqp + '/DBTJSQPInterface/doubleprize/lotteryPrize'; //抽奖接口
    var Port_Check = vge.jsqp + '/DBTJSQPInterface/user/getCaptcha'; //获取验证码接口
    var Port_Sub = vge.jsqp + '/DBTJSQPInterface/user/savePrize'; //大奖提交接口
    var Port_QueryList = vge.jsqp + '/DBTJSQPInterface/gifts/queryPrizeList'; //大奖列表查询接口
    var Port_FindPrize = vge.jsqp + '/DBTJSQPInterface/user/findPrize'; //一等奖查询接口
    var Port_Rule = vge.jsqp + '/DBTJSQPInterface/doubleprize/findActivity'; //活动说明接口
        ...
        setTimeout(function () {
            ...
            $('.cash').fadeIn(600, function () {
                ...
                if (doublePrize.doublePrizeLottery == '1') { //本次是否中奖
                    ...
                    // 累计支数
                    $('.doubleAlertNum').text(doublePrize.doublePrizeScanNum);
                    setTimeout(function () {
                        $('.doubleAlert').show();
                    }, 2000);
                } else {
            ...
            success: function (jo, status, xhr) {
                if (jo.result.code == '0') {
                    if (jo.result.businessCode === '0') {
                        ...
                        $('.doubleGetPrizeImg').attr('src', jo.reply.prizeImg);
                        $('.doubleGet').show();
                        $('.doubleAlert').hide();
                        prizeSource = true;
    ...
    // 抽奖后未中奖按钮
    $('.doubleNoneBtn').one('click', function () {
        $('.double').hide();
    });
    ...
    function getYzcode() {
        ...
        } else if (!reg2.test($('.doubleIdcard').val()) || !getIdcardValidateCode($('.doubleIdcard').val())) {
