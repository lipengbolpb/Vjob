(function () {
    ...
    dom_qrbtn.addEventListener('click', function(ev) {
        ...
        canvas.width = size;
        canvas.height = size;
        qrcode.makeCode('HTTP://VJ1.TV/I/'+$('#qrcode').val());
        ...
        vge.ajxget(requrl, 5000, function (r) {
            try {
                ...
                if (o.subscribe == '0') { //未关注
                    window.location.replace('http://' + location.host + '/v/jxqp-standard/attention.html');
                } else { //已关注用户
                    // window.location.reload();
                    window.location.replace('http://' + location.host + '/jxqp-standard/too/mybag');
                }
            } catch (e) {
