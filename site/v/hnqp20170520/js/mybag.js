(function() {
	var show = document.getElementById("show"),
		...
		mybag_bottom = document.getElementsByClassName('mybag_bottom')[0],
		...
		tx_btn = document.getElementById("btn"),
		dom_mask = document.getElementById('wxmark'),
		dom_num = document.getElementById('num'),
		dom_curmoney = document.getElementById('curmoney'),
	...
	function attentioned() {
		...
		function onepage_note(currentpage, cb) {
			var javai = vge.hnqp + '/DBTHuaNQPInterface/gifts/queryAllGiftsList'; //华南红包列表接口
			var req = {
				"openid": openid,
				"hbopenid": hbopenid,
			...
			vge.clog('debug', [javai, JSON.stringify(req)]);
			vge.callJApi(javai, req, function(jo) {
				if(jo.result.code === '0') {
					if(jo.result.businessCode === '0') { //  jo.reply.objList!==undefined && jo.reply.objList.length>0
						if(currentpage == 1) {
							dom_num.innerHTML = jo.reply.total;
							dom_curmoney.innerHTML = Number(jo.reply.totalMoney).toFixed(2);
							dom_totmoney.innerHTML = Number(jo.reply.giftsMoney).toFixed(2);
						}
						var i = 0,
						...
						dom_num.innerHTML = '0';
						dom_totmoney.innerHTML = '0.00';
						dom_curmoney.innerHTML = '0.00';
						if(first) {
							dom_more.innerHTML = '仅显示近30天的记录';
							dom_more.style.fontSize = '0.6rem';
							dom_more.removeEventListener('click', getm, false);
