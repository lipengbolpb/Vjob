(function () {
    ...
    $(document).ready(function () {
      //会员日礼盒酒 青啤1903瓶一箱+6听复古罐
      if (grandPrizeType == 'Q' || grandPrizeType == 'q') {
        newVipPrize()
      } else if (grandPrizeType == 'P' || grandPrizeType == 'p') {
        // 礼盒酒一箱
      } else {
        return
      }
  ...
  //会员日14礼盒酒
    function newVipPrize() {
      $('.get .top').css("backgroundImage", "url(img/prize-bg2.jpg)");
      $('.get .ck').addClass('btn-shadow');
      $('#prize-img').attr("src","img/prize2.png").css({
        marginTop: '-1.3rem',
        width: '14rem'
      });
      $('.info .slogan').attr("src","img/prize-slogan2.png").css('marginTop','-1.5rem');
      $('.title p').text('幸运奖');
      $('.content').html('<p style="margin-top:0.2rem;">青岛啤酒高端品鉴酒一套</p>' + '<p style="font-size: 0.5rem">活动区域：辽宁省' + '<br>活动时间：2019年8月19日'+ '<br>兑奖截止时间：2019年12月31日24:00止</p>');
      $('.ckff_box').css("marginTop", '1.5rem');
      $('.ckff_box a').eq(0).css("display", 'none');
      $('.ckff_box a').eq(1).text('查看兑奖电话>>');
      $('.alert').html('温馨提示：<br> 幸运奖为青岛啤酒增投奖项，请中奖消费者保存好瓶盖或拉环， 保持通讯畅通，请耐心等待主办方与您联系。');
