$(document).ready(function () {
    // 接口
    var Port_Check = vge.lnqp + '/DBTLNQPInterface/user/getCaptcha'; //获取验证码接口
    var Port_Sub = vge.lnqp + '/DBTLNQPInterface/user/savePrize'; //大奖提交接口
    var Port_QueryList = vge.lnqp + '/DBTLNQPInterface/gifts/queryPrizeList'; //大奖列表查询接口
    var Port_FindPrize = vge.lnqp + '/DBTLNQPInterface/user/findPrize'; //一等奖查询接口
    ...
    function initList(list,flag) { //显示列表
        ...
        var tpl3 = '<li>' +
            '<img src="{{prizeImg}}">' +
            '<div><p>{{prizeName}}</p><p>领奖时间：<span>{{useTime}}</span></p></div>' +
            '<img src="/v/hbqp-common2.0/img/double/prize-line.png">' +
                        ...
                        success: function (jo, status, xhr) {
                            if (jo.result.code === '0') {
                                if (jo.result.businessCode === '0') {
									...
									$('.przVcode').html('中奖码:'+jo.reply.prizeVcode);
    ...
    function getYzcode() {
        ...
        } else if ($('.doubleAddress').val() === '' || $('.doubleAddress').val()=== ' ') {
