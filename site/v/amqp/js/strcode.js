...
　　　　$('body').height($('body')[0].clientHeight);
    ...
    function only_strcode () {
        ...
        str_code.addEventListener('keyup',function () {
            if (reg1.test(str_code.value)) {
                dom_btn.style.backgroundColor='#ffe036';
                dom_btn.style.color = '#cf3c35';
            } else{
    ...
    function getCheckCode(cb) {//获取验证码
        var javai = vge.amqp + '/DBTBYMQPInterface/user/getCaptcha';
        var req = {
    ...
    function init () {
        var javai = vge.amqp + '/DBTBYMQPInterface/sweep/getFailCount';
        var req = {
            "openid":openid
        };
        vge.callJApi(javai, req, function(jo) {
    ...
    function sendcode() {
        var javai = vge.amqp + '/DBTBYMQPInterface/sweep/serialCode';
        var req = {
            "openid":openid,
            // "unionid":unionid,
            "serialcode":scode,
        ...
        vge.callJApi(javai, req, function(jo) {
            if (jo.result.code ==='0') {
                ...
                case '0':
                    ...
                    location.replace('http://' + location.host + '/amqp/txo/getcash?bizcode='+jo.result.businessCode);
                    break;
                case '1':// 1 - 该积分码不存在"
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '这个串码不存在';
                    dom_word.innerHTML = '仔细核对盖码哦~';
                    break;
                case '2':// 2 - 该积分码已经被使用过
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '这个串码已被扫';
                    dom_earnTime.innerHTML = jo.reply.earnTime;
                    document.getElementsByClassName('time')[0].style.display = 'block';
                    dom_word.innerHTML = '再扫一瓶试试看~';
                    break;
                case '3':// 3 - 积分码已过期
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '这个串码已过期';
                    dom_word.innerHTML = '这么好的啤酒要学会珍惜~';
                    break;
                case '4':// 4 - 活动未开始
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '活动未开始';
                    dom_word.innerHTML = '亲再等等哦~';
                    dom_batch.innerHTML = jo.reply.batchName+'<br />服务热线：18519813990'
                    break;
                case '5':// 5 - 活动已结束
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '活动已截止';
                    dom_word.innerHTML = '下次早点来哦~';
                    break;
                case '6':// 6 - 积分码异常(通常为服务器报错)
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '系统升级中';
                    dom_word.innerHTML = '亲稍安勿躁，敬请期待~';
                    break;
                case '7':              // 大奖
                    ...
                    location.replace('http://' + location.host + '/v/amqp/prize.html?bizcode='+jo.result.businessCode);
                    break;
                case '8':// 8-需要验证码
                ...
                case '10': // 10-错误超过6次,请明天再试
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '错误次数过多';
                    dom_word.innerHTML = '明天再来试试吧';
                    break;
                case '11':
                    sessionStorage.totalAccountMoney = jo.reply.totalAccountMoney;
                    sessionStorage.currentMoney = jo.reply.currentMoney;
                    ...
                    break;
                case '12':              // 可疑用户
					location.replace('http://' + location.host + '/v/amqp/getMsg.html?bizcode='+jo.result.businessCode);
		            break;    
	        	case '13':              // 黑名单
					location.replace('http://' + location.host + '/v/amqp/getMsg.html?bizcode='+jo.result.businessCode);
		            break;
                ...
                case '-1':
                    mark.style.display = 'block';
                    dom_notice.innerHTML = '系统开了个小差';
                    dom_word.innerHTML = '酒香不怕巷子深，稍后再试吧';
                    break;
                default:
                    if(jo.reply){
                    ...
                    location.replace('http://' + location.host + '/v/amqp/fail.html?bizcode='+jo.result.businessCode);
                }
            }else{//code!='0'
                title_tip('尊敬的用户','呜呜，系统开了个小差，请稍后重试！','我知道了');
            }
