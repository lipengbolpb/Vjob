(function () {
    ...
    var flag = true;
    // $('#info').html('您扫的码是【'+qr+'】<br>openid: '+openid+'<br>amqpappid:'+vge.amqpappid);
    ...
    loading('玩命加载中');
	location.replace('http://' + location.host + '/v/amqp/active.html');
    ...
    function sweep() {
        if (flag) {
            flag = false;
            loading('玩命加载中'); //调用接口
            var japi = vge.amqp + '/DBTBYMQPInterface/sweep/sweepQrcode';
            var req = {
                "openid": openid,
                "sweepstr": qr,
            ...
            switch (jo.result.businessCode) {
                ...
                case '7': // 大奖
                    ...
                    if (jo.reply.grandPrizeType == '1') {
                        ...
                        location.replace('http://' + location.host + '/v/amqp/prize2.html?bizcode=' + jo.result.businessCode);
                    } else {
