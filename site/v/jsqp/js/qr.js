(function() {
	...
	function sweep() {
		if(flag) {
			flag = false;
			loading('玩命加载中'); //调用接口
			var japi = vge.jsqp + '/DBTJSQPInterface/sweep/sweepQrcode';
			var req = {
				"openid": openid,
				"sweepstr": qr,
		...
		if(jo.result.code == '0') {
			if(jo.reply){
				if(jo.reply.activityVersion == '1') { //经典
					project = 'jsqp';
				} else if(jo.reply.activityVersion == '2') { //崂山
			...
			switch(jo.result.businessCode) {
				...
				case '7': //一等奖或二等奖
					...
					//特等奖类别
					sessionStorage.grandPrizeType = jo.reply.grandPrizeType === undefined ? '' : jo.reply.grandPrizeType;
					if(jo.reply.grandPrizeType=='1'||jo.reply.grandPrizeType=='2'||jo.reply.grandPrizeType.toUpperCase()=='P'){
