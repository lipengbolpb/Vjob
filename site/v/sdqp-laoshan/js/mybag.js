(function () {
    ...
    var args = vge.urlparse(location.href),
        openid = '',
        hbopenid = '';
    if(sessionStorage.openid){ // from扫码来红包
    ...
    if (location.href.indexOf('/sdqp-prs/txo/mybag') != -1) { //扫码打开
    ...
    function onepage_note(needAuto,cb) {
                ...
                if (jo.result.code === '0') {
                    if (jo.result.businessCode === '0') {
                        ...
                        if (l === 0 || lst === undefined) {
                            if (first) {
                                first = false;
                                $('#mon_list').html('<div class="no_list"><span class="nothing-text">仅显示近30天的记录</span></div>');
                            }
                            next = false;
                            if (cb !== undefined) {
                                cb();
        ...
        function showPrizeList(list) {
            ...
            if ((list.length == 0 || list.length == undefined) && currentpagePrize == 1) {
                $('.list_prize').html('<div class="no_list"><span class="nothing-text">您还没有大奖记录哦~</span></div>')
            }
