(function () {
    ...
    $(document).ready(function () {
        if (grandPrizeType == 'P' || grandPrizeType == 'p') {//青啤有一套
			...
			$('.change-content ul').css('display','none');
			$('.change-content .bigPrize').css('display','block');
			$('.warning').css({'backgroundImage':'url(/v/nmqp/img/warning_2.png)','height':'3.875rem'});
			$('.warning p').html('重要提示:自用户中奖之日起（不包含中奖日）48小时内，需完成奖项兑换，填写中奖信息并提交。因涉及办理相关手续，若超出时限未提交中奖信息，则视为自动放弃该奖项所有权。详情请咨询 15771551903，敬请知晓！');
			$('.warning p').css({'color':'#ffff99','text-decoration': 'underline'})
			init();
