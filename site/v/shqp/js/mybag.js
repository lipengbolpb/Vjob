(function() {
	...
	show.addEventListener('click', function() {
		_hmt.push(['_trackEvent', 'click', '10.1我的零钱包页', '详细记录']);
		document.getElementsByClassName("mybag_top")[0].style.marginTop = -H + "px";
	}, false);

	hide.addEventListener("click", function() {
		_hmt.push(['_trackEvent', 'click', '11.1扫码详细记录页', '收起']);
	...
	document.getElementsByClassName('lt')[0].addEventListener('click', function() {
		_hmt.push(['_trackEvent', 'click', '10.3我的零钱包页', '联通']);
	...
	document.getElementsByClassName('xc')[0].addEventListener('click', function() {
		_hmt.push(['_trackEvent', 'click', '10.4我的零钱包页', '携程']);
	...
	if (location.href.indexOf('/shqp/txo/mybag') != -1) { //扫码打开
	...
	} else if (location.href.indexOf('/shqp/tto/mybag') != -1) { //推送打开
		onepage_note(true);
		queryPrizeList();
	} else { //公众号打开
		onepage_note();
	...
	dom_btn.addEventListener('click', function() {
		_hmt.push(['_trackEvent', 'click', '10.2我的零钱包页', '立即提现']);
		ifremeber(false);
	});
	...
	dom_mask.addEventListener('click', function() {
		_hmt.push(['_trackEvent', 'click', '9.1入账通知提示页', '我知道了']);
