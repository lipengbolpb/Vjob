vge.register('vge', function(g) {
	...
	z.callJApi=function(api, params, callback, log){
		...
		vge.ajxpost(api, ps, 30000, function(r) {
			try {
				var jo = JSON.parse(r);
				if (jo.result.code === '0') {
                    if(log!==undefined) z.clog('call debug ',[api,ps,r]);
					if(z.isFunction(callback)) callback(jo);
				} else {
					z.clog('err1msg ',[api,ps,r]);
                    // clog: api, ps, r
				}
			} catch (e) {
				z.clog('err2msg ',[api,e,r]);
			}
		},function(e){
			z.clog('err3msg ',[api,e,ps]);
		});
	};
    ...
    z.clog=function(type,arr){
        ...
        z.ajxget('/clog?'+type+tst+url+'='+ msg, 5000, function(r) {});
    };
