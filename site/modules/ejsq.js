...
exports.render = function (template, options, req,res) {
	var fpath = tpldir + template,
        arr = template.split('/'),
        respath = '/v';
    if(arr.length>1) respath = `${respath}/${arr[0]}`; // css,img,js 静态资源相对路径处理
		...
		if(stats.isFile()) {
			...
			if (bm && lastModified===req.headers['if-modified-since']) { // Etag
                ...
                try{
                    ...
                    let output= beforeRender(html);
                    // console.log(html);
                    html = afterRender(output.html.join(''), output);
                    // console.log(html);

                    html = html.replace(recss, `"${respath}/css/`);
                    html = html.replace(reimg, `"${respath}/img/`);
                    html = html.replace(rejs, `"${respath}/js/`);
	                res.end(html);
                }catch(e){
                    console.log('500 服务器模板参数错误',e);
