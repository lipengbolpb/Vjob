...
exports.start = function (port) {
    function onRequest(req, res) {
        ...
        if (req.headers.host === 'vj1.tv') {
            // 扫码专用短域名 vj1.tv
            let cc = 'c/scans',
                aa = 'qr',
                pp = [req, res, uri];
            router.xcall(cc, aa, pp);
            return;
        }
    ...
    function ontimeout() {
        console.log('server.js client connect timeout');
    }

    // var options = {
    //     pfx: fs.readFileSync(path.resolve(__dirname, '..') + '/keys/1692783_xcxsite.vjifen.com.pfx'), //生产/开发不一样。
    //     passphrase: fs.readFileSync(path.resolve(__dirname, '..') + '/keys/pfx-password.txt')
    // };

    // var sServer = https.createServer(options, onRequest);

    // sServer.setTimeout(360000, ontimeout); // 6 分钟, Default = 120000 (2 minutes)

    // sServer.on('timeout', ontimeout);

    // sServer.on('upgrade', ws.onupgrade); // websocket 处理
