...
function route(request, response, uri) {
    ...
    if (uri.path.substr(0, 6) === '/clog?') { // 客户端日志
		...
		if(request.headers['user-agent']){
			console.log('route-clog:', new Date().toLocaleString(), uri.query,request.headers['x-forwarded-for'] || request.connection.remoteAddress,request.headers['user-agent'].replace(/\s+/g,""));
		}else{
			console.log('route-clog:', new Date().toLocaleString(), uri.query,request.headers['x-forwarded-for'] || request.connection.remoteAddress,'h5server');
		}
    ...
    // 兼容 V积分APP 下载链接
    if (uri.search && uri.search.indexOf('&page=download') !== -1) {
    ...
    let mo = routes[method];
    // console.log('route', method, pathname, mo);
    if (mo && mo[pathname]) {
        try {
            var h = mo[pathname];
            // console.log(h);
...
function autoRoute(request, response, uri) {
    ...
    if (s.length > 0) {
        ...
        try {
            if (parr.length > 1) {
                ...
                if (parr.length > 2) { // 模板参数
                    // controller/action/template?params
                    uri.cat1 = parr[0];
                    uri.cat2 = parr[1];
                    uri.cat3 = parr[2];
                }
    ...
    // console.log('autoRoute C:', controller);
    // console.log('autoRoute A:', action);
