...
var cdirs = ['/static/', '/v/', '\\static\\', '\\v\\', '/qr/', '\\qr\\', '/.well-known/', '\\.well-known\\', '/V/', '\\V\\']; // 允许客户端访问的目录白名单
...
exports.r = function (fpath, ext, contentType, req, res) {
    ...
    if (ext !== 'ico' && // except favicon.ico
        fpath.indexOf(cdirs[4]) !== 0 &&
        fpath.indexOf(cdirs[5]) !== 0 &&
        fpath.indexOf(cdirs[6]) !== 0 &&
        fpath.indexOf(cdirs[7]) !== 0 &&
        fpath.indexOf(cdirs[8]) !== 0 &&
        fpath.indexOf(cdirs[9]) !== 0 &&
